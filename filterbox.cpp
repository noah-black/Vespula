...
FilterBox::FilterBox(Filter *filter, LFO *lfo, NoteFactory *noteFactory, QWidget *parent) :
    QGroupBox(parent),
    filter(filter),
    lfo(lfo),
    ...
    
{
    ...
    QObject::connect(&cutoffLfoAmountSelect, SIGNAL(valueChanged(int)), this, SLOT(setCutoffLfoAmount(int))); 
    QObject::connect(&cutoffLfoAmountSelect, SIGNAL(valueChanged(int)), this, SLOT(setFocus()));

    QObject::connect(&cutoffEnvAmountSelect, SIGNAL(valueChanged(int)), this, SLOT(setCutoffEnvAmount(int))); 
    QObject::connect(&cutoffEnvAmountSelect, SIGNAL(valueChanged(int)), this, SLOT(setFocus()));
}
