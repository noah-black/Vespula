...
Looper::Looper(NoteFactory *noteFactory, double periodSeconds)
    : Keyboard(noteFactory), savedNotes(periodSeconds * SAMPLE_RATE) {
...
double Looper::getSample() {
  ...
  if (!savedNotes[phase].empty()) {
    for (std::vector<struct savedNote>::iterator it = savedNotes[phase].begin();
         it != savedNotes[phase].end(); ++it)
      playSavedNote(*it);
  }
  phase++;
  phase = phase % period;
...
void Looper::releaseNoteInternal(enum note n) {
  if (lastNoteFor.find(n) != lastNoteFor.end()) {
    ...
    sNote.octave = octave;
    savedNotes[(phase - sNote.duration) % period < 0
                   ? (phase - sNote.duration) % period + period
                   : (phase - sNote.duration) % period]
        .push_back(sNote);
    note->release();
    lastNoteFor.erase(n);
  }
...
void Looper::playSavedNote(struct savedNote sNote) {
  Note *note;
  double freq;
  freq = freqs[getTransposition(sNote.note)] *
         pow(2.0, sNote.octave + ((double)transpose) / 12);
...
void Looper::clear() {
  pthread_mutex_lock(&noteMutex);
  pthread_mutex_lock(&mutexsum);
  for (std::vector<std::vector<struct savedNote>>::iterator it =
           savedNotes.begin();
       it != savedNotes.end(); ++it)
    it->clear();
  std::deque<Note *>::iterator it;
  it = notes.begin();
  while (it != notes.end()) {
