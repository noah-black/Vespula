...
Synthesizer::Synthesizer() : vibrato(&keyboard, 0.1, 1000) {
	unsigned int val;
	int dir, rc;
	initMaps();
	state = NOT_RUNNING;
...
void Synthesizer::setAttack(int i) {
	keyboard.setAttack(((double)i)/20);
}

void Synthesizer::setDecay(int i) {
	keyboard.setDecay(((double)i)/20);
}

void Synthesizer::setRelease(int i) {
	keyboard.setRelease(((double)i)/20);
}
