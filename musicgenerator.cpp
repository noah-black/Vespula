...
MusicGenerator::MusicGenerator(Keyboard **keyboard, NoteFactory *noteFactory,
                               WaveTable *waveTable)
    : keyboard(keyboard), noteFactory(noteFactory), waveTable(waveTable) {
  srand(time(NULL));
...
void MusicGenerator::advance() {
  ...
  if (rand() % (int)(SAMPLE_RATE) / 12 == 0) {
    ...
    durationInSamples =
        rand() % ((int)SAMPLE_RATE * 4) + (int)(SAMPLE_RATE / 4);
