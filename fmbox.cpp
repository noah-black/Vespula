...
FmBox::FmBox(NoteFactory *noteFactory, QWidget *parent)
    : QGroupBox(parent), noteFactory(noteFactory), fmLabel(this),
      fmDepthLabel(this), fmDepthSelect(this), fmEnabled(this),
      fmEnvelopeLabel(this), fmEnvAmountSelect(this), fmEnvelopeEnabled(this),
      fmLfoLabel(this), fmLfoAmountSelect(this), fmLfoEnabled(this) {
  ...
  QObject::connect(&fmDepthSelect, SIGNAL(valueChanged(int)), this,
                   SLOT(setFmDepth(int)));
  QObject::connect(&fmDepthSelect, SIGNAL(valueChanged(int)), this,
                   SLOT(setFocus()));

  QObject::connect(&fmEnabled, SIGNAL(stateChanged(int)), this,
                   SLOT(setFmEnabled(int)));
  QObject::connect(&fmEnabled, SIGNAL(stateChanged(int)), this,
                   SLOT(setFocus()));

  QObject::connect(&fmEnvAmountSelect, SIGNAL(valueChanged(int)), this,
                   SLOT(setFmEnvAmount(int)));
  QObject::connect(&fmEnvAmountSelect, SIGNAL(valueChanged(int)), this,
                   SLOT(setFocus()));

  QObject::connect(&fmEnvelopeEnabled, SIGNAL(stateChanged(int)), this,
                   SLOT(setFmEnvelopeEnabled(int)));
  QObject::connect(&fmEnvelopeEnabled, SIGNAL(stateChanged(int)), this,
                   SLOT(setFocus()));

  QObject::connect(&fmLfoAmountSelect, SIGNAL(valueChanged(int)), this,
                   SLOT(setFmLfoAmount(int)));
  QObject::connect(&fmLfoAmountSelect, SIGNAL(valueChanged(int)), this,
                   SLOT(setFocus()));

  QObject::connect(&fmLfoEnabled, SIGNAL(stateChanged(int)), this,
                   SLOT(setFmLfoEnabled(int)));
  QObject::connect(&fmLfoEnabled, SIGNAL(stateChanged(int)), this,
                   SLOT(setFocus()));
}

void FmBox::setFmDepth(int i) { noteFactory->setFmDepth(((double)i) * 100); }
